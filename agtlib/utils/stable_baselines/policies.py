...
from .preprocessing import get_action_dim, is_image_space, maybe_transpose, preprocess_obs
...
from .type_aliases import PyTorchObs, Schedule
from .utils import get_device, is_vectorized_obs
...
class BaseModel(nn.Module):
    ...
    def is_vectorized_observation(self, observation: Union[np.ndarray, Dict[str, np.ndarray]]) -> bool:
        ...
        if isinstance(observation, dict):
            ...
            for key, obs in observation.items():
                obs_space = self.observation_sself.pace.spaces[key]
                vectorized_env = vectorized_env or self.is_vectorized_observation(maybe_transpose(obs, obs_space), obs_space)
        else:
    ...
    def obs_to_tensor(self, observation: Union[np.ndarray, Dict[str, np.ndarray]]) -> Tuple[PyTorchObs, bool]:
        ...
        if isinstance(observation, dict):
            ...
            for key, obs in observation.items():
                ...
                if is_image_space(obs_space):
                    ...
                    obs_ = np.array(obs)
                vectorized_env = vectorized_env or self.is_vectorized_observation(obs_, obs_space)
        ...
        if not isinstance(observation, dict):
            ...
            vectorized_env = self.is_vectorized_observation(observation, self.observation_space)
            # Add batch dimension if needed
            observation = observation.reshape((-1, *self.observation_space.shape))  # type: ignore[misc]

        obs_tensor = self.obs_as_tensor(observation, self.device)
...
class ActorCriticPolicy(BasePolicy):
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        actions = distribution.get_action(deterministic=deterministic)
    ...
    def _predict(self, observation: PyTorchObs, deterministic: bool = False) -> th.Tensor:
        ...
        :return: Taken action according to the policy
        """
        return self.get_distribution(observation).get_action(deterministic=deterministic)
