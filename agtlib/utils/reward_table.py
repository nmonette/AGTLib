...
def calculate_reward(state, action, n_agents, dim):
    ...
    for agent in range(1, n_agents+1):
        ...
        if not state["goal1_term"] and action_map[action[agent-1]](state[f"{agent}_x"], state[f"{agent}_y"]) == (state["goal1_x"], state["goal1_y"]):
            total_reward += -1 if agent-1 < n_agents - (n_agents // 2)  else 1

        if not state["goal2_term"] and action_map[action[agent-1]](state[f"{agent}_x"], state[f"{agent}_y"]) == (state["goal2_x"], state["goal2_y"]):
            total_reward += -1 if agent-1 < n_agents - (n_agents // 2)  else 1
...
def generate_reward_3x3():
    dims = [3, 3, 2, 3, 3, 2, 3, 3, 2, 3, 3, 2, 3, 3, 2, 4, 4, 4]
                                                                ...
                                                                for a1 in range(4):
                                                                    for a2 in range(4):
                                                                        for a3 in range(4):
                                                                            ...
                                                                            table[x1, y1, term_1, x2, y2, term_2, x3, y3, term_3,
                                                                                  goal1_x, goal1_y, goal1_term, goal2_x, goal2_y, 
                                                                                  goal2_term, a1, a2, a3] = calculate_reward(state, (a1, a2, a3), 3, 3)
    
    table = table.reshape(torch.prod(torch.tensor([3, 3, 2, 3, 3, 2, 3, 3, 2, 3, 3, 2, 3, 3, 2])), 4, 16)
