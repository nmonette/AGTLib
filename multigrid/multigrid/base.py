...
from .core.actions import Action, BallAction
...
class MultiGridEnv(gym.Env, RandomMixin, ABC):
    ...
    def __str__(self):
        ...
        # Get agent locations
        location_to_agent = {tuple(agent.pos): agent for agent in self.agents if not agent.terminated}
...
class TeamMultiGridEnv(MultiGridEnv):
    goals: int = 2
    ...
    def __init__(
        ...
        tile_size: int = TILE_PIXELS,
        agent_pov: bool = False):
        super().__init__(mission_space, agents, grid_size, width, height, max_steps, see_through_walls,
                         agent_view_size, allow_agent_overlap, joint_reward, success_termination_mode, 
                         failure_termination_mode, render_mode, screen_size, highlight, tile_size, 
                         agent_pov)
        self.actions = BallAction
    ...
    def observation_space(self) -> spaces.Dict[AgentID, spaces.Space]:
        ...
        obs = {}
        for i in range(self.num_agents):
            obs[f"{i}_x"] = spaces.Discrete(self.grid_size - 2)
            obs[f"{i}_y"] = spaces.Discrete(self.grid_size - 2)
            obs[f"{i}_terminated"] = spaces.Discrete(2)
                
        obs["goal1_x"] = spaces.Discrete(self.grid_size - 2)
        obs["goal1_y"] = spaces.Discrete(self.grid_size - 2)
        obs["goal1_terminated"] = spaces.Discrete(2)

        obs["goal2_x"] = spaces.Discrete(self.grid_size - 2)
        obs["goal2_y"] = spaces.Discrete(self.grid_size - 2)
        obs["goal2_terminated"] = spaces.Discrete(2)

        # final = {}
        # for i in range(self.num_agents):
        #     temp = obs.copy()
        #     for j in range(self.num_agents):
        #         temp[f"is_{j}"] = spaces.Discrete(1)
        #     final[i] = temp
        final = {i:obs for i in range(self.num_agents)}
        
        return spaces.Dict({i:spaces.Dict(j) for i,j in final.items()})
    ...
    def handle_actions(
        ...
        for i in order:
            ...
            # Go left
            if action == BallAction.left:
                agent.state.dir = 2

            # Go right
            elif action == BallAction.right:
                agent.state.dir = 0

            # Go down
            elif action == BallAction.up:
                agent.state.dir = 3

            # Go up
            elif action == BallAction.down:
                agent.state.dir = 1
            ...
            if fwd_obj is not None and fwd_obj.type == Type.wall:
                continue
    ...
    def on_success(
        ...
        terminations : dict[AgentID, bool]
            Termination dictionary to be updated
        """
        self.grid.set(agent.state.pos[0],agent.state.pos[1],None)

        agent.state.terminated = True
        terminations[agent.index] = True
       
        winner = 0 if agent.index < self.num_agents - (self.num_agents // 2) else 1
        team1 = range(0, self.num_agents - (self.num_agents // 2))
        team2 = range(self.num_agents - (self.num_agents // 2), self.num_agents)
    ...
    actions: dict[AgentID, Action]) -> tuple[
        ...
        # Generate outputs
        observations = self.gen_obs(step=True)
        terminations = dict(enumerate(self.agent_states.terminated))
        truncated = self.step_count >= self.max_steps
        truncations = dict(enumerate(repeat(truncated, self.num_agents)))
    ...
    def gen_obs(self) -> dict[AgentID, ObsType]:
        ...
        """
        if step:
            # making the grid fully observable 
            image = [self.grid.state for i in range(self.num_agents)]
            observations = {}
            for i in range(self.num_agents):
                observations[i] = np.concatenate((image[i].flatten(),np.array([direction[j] for j in range(self.num_agents)]),np.array([i == j for j in range(self.num_agents)], dtype=np.int8)))
        else:
        """
        image = gen_obs_grid_encoding(
            self.grid.state,
    ...
    def reset(self, seed: int | None = None, **kwargs) -> tuple[
        dict[AgentID, ObsType]:
        dict[AgentID, dict[str, Any]]]:
        """
        Reset the environment.
