...
from .agent import Agent, BallAgent
...
class Grid:
    ...
    def render_tile(
        ...
        if agent and not agent.state.terminated:
    ...
    def render(
        ...
        location_to_agent = defaultdict(
            type(None),
            {tuple(agent.pos): agent for agent in agents if not agent.terminated}
        )
        ...
        for j in range(0, self.height):
            for i in range(0, self.width):
                ...
                cell = self.get(i, j)
                # gotta see what happens when there is multiple agents in the same location
